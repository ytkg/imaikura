...
describe("BookCard", () => {
  it("requests moving an included book to held", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "テストの本を保留にする" }),
    );
  ...
  it("edits title and price and marks the price as changed", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "書籍情報と価格を編集" }),
    );
    fireEvent.change(screen.getByLabelText("タイトル"), {
      target: { value: "店頭の本" },
    });
    fireEvent.change(screen.getByLabelText("税抜価格"), {
      target: { value: "1250" },
    });
  ...
  it("offers the original OpenBD price for an edited book", () => {
    render(
      <BookCard
        book={{ ...baseBook, price: 1250, priceEdited: true }}
        onUpdate={vi.fn()}
        onRemove={vi.fn()}
        onRetry={vi.fn()}
      />,
