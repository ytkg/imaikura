...
export async function fetchOpenBdBook(isbn: string, signal?: AbortSignal): Promise<OpenBdBook | null> {
  const response = await fetch(`${API_URL}?isbn=${encodeURIComponent(isbn)}`, { signal })
  if (!response.ok) throw new Error(`OpenBD request failed: ${response.status}`)
  ...
  const author = typeof summary.author === 'string' ? summary.author.trim() : ''

  return {
    title: typeof summary.title === 'string' ? summary.title : '',
    authors: author ? author.split(/\s*[／/]\s*/).filter(Boolean) : [],
    coverUrl: typeof summary.cover === 'string' && summary.cover ? summary.cover : null,
