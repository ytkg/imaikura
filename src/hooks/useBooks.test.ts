...
describe("useBooks", () => {
  ...
  it("moves a book between included and held collections", async () => {
    ...
    expect(result.current.plannedBooks).toHaveLength(0);
  ...
  it("preserves an edited price when metadata is retried", async () => {
    ...
    await waitFor(() => expect(result.current.books[0].status).toBe("ready"));
    act(() =>
      result.current.updateBook("9784101010014", {
        price: 1500,
        priceEdited: true,
      }),
    );
    ...
    await act(async () => {
      await result.current.retryBook("9784101010014");
    });
    expect(result.current.books[0]).toMatchObject({
      openBdPrice: 1300,
      price: 1500,
  ...
  it("distinguishes missing records from network errors", async () => {
    ...
    await waitFor(() =>
      expect(
        result.current.books.every((book) => book.status !== "loading"),
      ).toBe(true),
    );
    expect(result.current.books.map((book) => book.status).sort()).toEqual([
      "error",
      "not-found",
    ]);
  });
